...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            elif self.can_contain(place.ant) and not self.ant_contained:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place.is_hive != True:
            if self.min_range <= self.entrance_counter <= self.max_range and len(current_place.bees) > 0:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        copy_current_bees1 = self.place.bees[:]
        print("DEBUG:", copy_current_bees1)

        for x in copy_current_bees1:
            x.reduce_health(amount)

        copy_current_bees2 = self.place.bees[:]
        super(FireAnt, self).reduce_health(amount)

        if self.health <= 0:
            for x in copy_current_bees2:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        elif self.chew_countdown == 0 and len(self.place.bees) > 0:
...
class TankAnt(ContainerAnt):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(2, **kwargs) #Health is 2
        self.ant_contained = None
    
    def action(self, gamestate):
        
        if self.ant_contained: #if containing an ant, ant does action as well
                self.ant_contained.action(gamestate)

        if len(self.place.bees):
            copy_current_bees1 = self.place.bees[:]
            for bee in copy_current_bees1: #For each bee in its place, do damage
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.can_have_queen == False:
            return 
        gamestate.can_have_queen = False
    ...
    def action(self, gamestate):
        ...
        places_behind = self.place.exit

        while type(places_behind) != AntHomeBase:
            if places_behind.ant:
                if places_behind.ant.is_container == True:
                    if places_behind.ant.ant_contained:
                        places_behind.ant.ant_contained.buff()
                places_behind.ant.buff()
            places_behind = places_behind.exit
